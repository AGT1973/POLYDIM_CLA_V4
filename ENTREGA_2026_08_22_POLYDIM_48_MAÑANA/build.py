# build.py
# Script Estándar de Compilación Nativa para DLLs (C++ y Rust) - POLYDIM V48
# ============================================================================
...
def ensure_source_files():
    cpp_src = os.path.join(BASE_DIR, "slerp_kernel_v47_matrix_free.cpp")
    rs_src = os.path.join(BASE_DIR, "lib_v47_matrix_free.rs")

    # Si falta el fuente C++, intenta extraerlo del monolito si está cargado
    if not os.path.exists(cpp_src):
        try:
            import polydim_v48_monolito
            if hasattr(polydim_v48_monolito, 'SLERP_KERNEL_CPP_SOURCE'):
                with open(cpp_src, 'w', encoding='utf-8') as f:
                    f.write(polydim_v48_monolito.SLERP_KERNEL_CPP_SOURCE)
                print(f"[BUILD] Extraído slerp_kernel_v47_matrix_free.cpp desde el monolito")
        except Exception:
            pass
    ...
    if not os.path.exists(rs_src):
        try:
            import polydim_v48_monolito
            if hasattr(polydim_v48_monolito, 'LIB_V47_RUST_SOURCE'):
                with open(rs_src, 'w', encoding='utf-8') as f:
                    f.write(polydim_v48_monolito.LIB_V47_RUST_SOURCE)
                print(f"[BUILD] Extraído lib_v47_matrix_free.rs desde el monolito")
        except Exception:
            pass
...
def compile_cpp_dll():
    print("[BUILD] Compilando kernel C++ (slerp_kernel_v47_matrix_free.cpp)...")
    ensure_source_files()
    cpp_src = os.path.join(BASE_DIR, "slerp_kernel_v47_matrix_free.cpp")
    cpp_dll = os.path.join(BASE_DIR, "slerp_kernel_v47_matrix_free.dll" if sys.platform == "win32" else "slerp_kernel_v47_matrix_free.so")
...
def compile_rust_dll():
    print("[BUILD] Compilando módulo Rust Lock-Free MPMC (lib_v47_matrix_free.rs)...")
    ensure_source_files()
    rs_src = os.path.join(BASE_DIR, "lib_v47_matrix_free.rs")
    rs_dll = os.path.join(BASE_DIR, "lib_v47_matrix_free.dll" if sys.platform == "win32" else "lib_v47_matrix_free.so")
...
def main():
    print("=== INICIANDO BUILD SYSTEM NATIVO POLYDIM V48 ===")
    compile_cpp_dll()
    compile_rust_dll()
