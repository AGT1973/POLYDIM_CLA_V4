...
pub unsafe extern "C" fn polydim_rust_householder_reflect(
    ...
    let result = panic::catch_unwind(|| {
        ...
        let v_slice = slice::from_raw_parts(v_ptr, dim);
        let v_norm_sq: f64 = v_slice.iter().map(|&a| a * a).sum();
        if v_norm_sq < 1e-30 {
            if x_ptr != out_ptr {
                std::ptr::copy(x_ptr, out_ptr, dim);
            }
            return 0;
        }
        let dot_xv: f64 = {
            let x_slice = slice::from_raw_parts(x_ptr, dim);
            x_slice.iter().zip(v_slice.iter()).map(|(&a, &b)| a * b).sum()
        };
        let factor = 2.0 * dot_xv / v_norm_sq;
        
        for i in 0..dim {
            let xi = *x_ptr.add(i);
            *out_ptr.add(i) = xi - factor * v_slice[i];
        }
        0
    });
...
EXPORT_SYM int polydim_cpp_householder_reflect(const double* x, const double* v, double* out, size_t dim) {
    ...
    if (bytes > UINTPTR_MAX - o || bytes > UINTPTR_MAX - a || bytes > UINTPTR_MAX - b) return -4;

    // v/out deben estar separados. x/out pueden solaparse exactamente (a == o), pero no parcialmente
    if (b < o + bytes && o < b + bytes) return -2;
    if (a != o && a < o + bytes && o < a + bytes) return -2;
...
class CliffordRotors:
    ...
    def apply_spherical_rotor(x: jnp.ndarray, U: jnp.ndarray, V: jnp.ndarray, theta: float = 0.1) -> jnp.ndarray:
        ...
        # O(D) Gram-Schmidt ortogonalizacion adaptativa (Fix Vector 5 y 7)
        u_norm = jnp.maximum(safe_norm(U, keepdims=True), eps)
        u_orth = U / u_norm
        
        proj = safe_dot(u_orth, V, keepdims=True)
        v_resid = V - proj * u_orth
        v_norm = jnp.maximum(safe_norm(v_resid, keepdims=True), eps)
        v_orth = v_resid / v_norm
        
        dot_U = safe_dot(u_orth, x, keepdims=True)
        dot_V = safe_dot(v_orth, x, keepdims=True)
        
        c, s = jnp.cos(theta), jnp.sin(theta)
        rot_U = c * dot_U - s * dot_V
        rot_V = s * dot_U + c * dot_V
        
        delta = (rot_U - dot_U) * u_orth + (rot_V - dot_V) * v_orth
    ...
    def cayley_transform(A: jnp.ndarray) -> jnp.ndarray:
        ...
        # Eliminada regularización espuria que rompía unitariedad (Fix Vector 6)
        return jax.scipy.linalg.solve(I - A_skew, I + A_skew)
