...
def test_source_packaging_names_match_runtime_expectations(polydim, tmp_path):
    ...
    cpp_runtime = project / "kernel_cpp_v79.cpp"
    rust_runtime = project / "kernel_rust_v79.rs"
...
def test_native_householder_dispatch_is_real(polydim):
    ...
    # not be harmless. The current v79 implementation ignores it.
...
def test_cpp_alias_contract_is_not_partial(polydim):
    p = Path(polydim.__file__).resolve().parent / "kernel_cpp_v79.cpp.txt"
...
def test_pmtp_roundtrip_bytes_are_not_self_inconsistent(polydim, monkeypatch):
    ...
    # MAC location must be fixed by the format. v79 returns a hand-spliced blob;
